...
import {truncate, LogItemobject, LogItem, Plugin, GenericPlugin} from '../utils';

export type GithubConfig = Plugin<{
    host?: string;
    token: string;
...
export default GenericPlugin<GithubConfig>(
    {
    ...
    async function GithubPlugin(config) {
        function call<T>(url: string, opts: AxiosRequestConfig<T> = {}) {
            ...
            return axios.get<T>(`${config.host}${url}`, opts);
        }
        const {
            data: {login},
        } = await call<{login: string}>('/user');

        const query = `is:open author:${login}`;

        const {data: issues} = await call<GithubIssuesResponse>(`/search/issues?q=${query}`);
